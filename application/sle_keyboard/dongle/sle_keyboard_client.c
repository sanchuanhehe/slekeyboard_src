...
uint16_t get_sle_keyboard_conn_id(void) { return g_sle_keyboard_conn_id; }
...
static void sle_keyboard_client_sample_seek_result_info_cbk(
  ...
  if (strstr((const char *)seek_result_data->data, SLE_KEYBOARD_SERVER_NAME) !=
      NULL) {
    memcpy_s(&g_sle_keyboard_remote_addr, sizeof(sle_addr_t),
...
static void sle_keyboard_client_sample_seek_cbk_register(void) {
  g_sle_keyboard_seek_cbk.sle_enable_cb =
      sle_keyboard_client_sample_sle_enable_cbk;
  g_sle_keyboard_seek_cbk.seek_enable_cb =
      sle_keyboard_client_sample_seek_enable_cbk;
  g_sle_keyboard_seek_cbk.seek_result_cb =
      sle_keyboard_client_sample_seek_result_info_cbk;
  g_sle_keyboard_seek_cbk.seek_disable_cb =
      sle_keyboard_client_sample_seek_disable_cbk;
  sle_announce_seek_register_callbacks(&g_sle_keyboard_seek_cbk);
}

static void sle_keyboard_client_sample_connect_state_changed_cbk(
...
void sle_keyboard_client_sample_pair_complete_cbk(uint16_t conn_id,
                                                  const sle_addr_t *addr,
                                                  errcode_t status) {
  osal_printk("%s pair complete conn_id:%d, addr:%02x***%02x%02x\n",
              SLE_KEYBOARD_DONGLE_LOG, conn_id, addr->addr[ADDR_INDEX_0],
              addr->addr[ADDR_INDEX_4], addr->addr[ADDR_INDEX_5]);
  if (status == 0) {
...
static void sle_keyboard_client_sample_connect_cbk_register(void) {
  g_sle_keyboard_connect_cbk.connect_state_changed_cb =
      sle_keyboard_client_sample_connect_state_changed_cbk;
  g_sle_keyboard_connect_cbk.pair_complete_cb =
      sle_keyboard_client_sample_pair_complete_cbk;
  sle_connection_register_callbacks(&g_sle_keyboard_connect_cbk);
}

static void sle_keyboard_client_sample_exchange_info_cbk(
    uint8_t client_id, uint16_t conn_id, ssap_exchange_info_t *param,
    errcode_t status) {
...
static void sle_keyboard_client_sample_find_structure_cbk(
    uint8_t client_id, uint16_t conn_id, ssapc_find_service_result_t *service,
    errcode_t status) {
  osal_printk("%s find structure cbk client: %d conn_id:%d status: %d \r\n",
              SLE_KEYBOARD_DONGLE_LOG, client_id, conn_id, status);
  osal_printk(
      "%s find structure start_hdl:[0x%02x], end_hdl:[0x%02x], uuid len:%d\r\n",
      SLE_KEYBOARD_DONGLE_LOG, service->start_hdl, service->end_hdl,
      service->uuid.len);
  g_sle_keyboard_find_service_result.start_hdl = service->start_hdl;
  g_sle_keyboard_find_service_result.end_hdl = service->end_hdl;
  memcpy_s(&g_sle_keyboard_find_service_result.uuid, sizeof(sle_uuid_t),
           &service->uuid, sizeof(sle_uuid_t));
}

static void sle_keyboard_client_sample_find_property_cbk(
    uint8_t client_id, uint16_t conn_id, ssapc_find_property_result_t *property,
    errcode_t status) {
  osal_printk("%s sle_keyboard_client_sample_find_property_cbk, client id: %d, "
              "conn id: %d, operate ind: %d, "
              "descriptors count: %d status:%d property->handle %d\r\n",
              SLE_KEYBOARD_DONGLE_LOG, client_id, conn_id,
              property->operate_indication, property->descriptors_count, status,
              property->handle);
  g_sle_keyboard_send_param.handle = property->handle;
  g_sle_keyboard_send_param.type = SSAP_PROPERTY_TYPE_VALUE;
}

static void sle_keyboard_client_sample_find_structure_cmp_cbk(
    uint8_t client_id, uint16_t conn_id,
    ssapc_find_structure_result_t *structure_result, errcode_t status) {
  unused(conn_id);
  osal_printk("%s sle_keyboard_client_sample_find_structure_cmp_cbk,client "
...
static void
sle_keyboard_client_sample_write_cfm_cb(uint8_t client_id, uint16_t conn_id,
                                        ssapc_write_result_t *write_result,
                                        errcode_t status) {
  osal_printk("%s sle_keyboard_client_sample_write_cb, conn_id:%d client id:%d "
              "status:%d handle:%02x type:%02x\r\n",
              SLE_KEYBOARD_DONGLE_LOG, conn_id, client_id, status,
              write_result->handle, write_result->type);
}

static void sle_keyboard_client_sample_ssapc_cbk_register(
    ssapc_notification_callback notification_cb,
    ssapc_notification_callback indication_cb) {
  g_sle_keyboard_ssapc_cbk.exchange_info_cb =
      sle_keyboard_client_sample_exchange_info_cbk;
  g_sle_keyboard_ssapc_cbk.find_structure_cb =
      sle_keyboard_client_sample_find_structure_cbk;
  g_sle_keyboard_ssapc_cbk.ssapc_find_property_cbk =
      sle_keyboard_client_sample_find_property_cbk;
  g_sle_keyboard_ssapc_cbk.find_structure_cmp_cb =
      sle_keyboard_client_sample_find_structure_cmp_cbk;
  g_sle_keyboard_ssapc_cbk.write_cfm_cb =
      sle_keyboard_client_sample_write_cfm_cb;
...
void sle_keyboard_client_init(ssapc_notification_callback notification_cb,
