...
static uint8_t g_sle_property_value[OCTET_BIT_LEN] = {0x0, 0x0, 0x0,
                                                      0x0, 0x0, 0x0};
...
static uint8_t g_sle_keyboard_base[] = {0x37, 0xBE, 0xA8, 0x80, 0xFC, 0x70,
...
static void sle_uuid_set_base(sle_uuid_t *out) {
  if (memcpy_s(out->uuid, SLE_UUID_LEN, g_sle_keyboard_base, SLE_UUID_LEN) !=
      EOK) {
...
static void ssaps_mtu_changed_cbk(uint8_t server_id, uint16_t conn_id,
                                  ...
                                  errcode_t status) {
  sample_print("%s ssaps ssaps_mtu_changed_cbk callback server_id:%x, "
               "conn_id:%x, mtu_size:%x, status:%x\r\n",
               SLE_KEYBOARD_SERVER_LOG, server_id, conn_id, mtu_size->mtu_size,
               status);
...
static void ssaps_start_service_cbk(uint8_t server_id, uint16_t handle,
                                    errcode_t status) {
  sample_print(
      "%s start service cbk callback server_id:%d, handle:%x, status:%x\r\n",
      SLE_KEYBOARD_SERVER_LOG, server_id, handle, status);
}
static void ssaps_add_service_cbk(uint8_t server_id, sle_uuid_t *uuid,
                                  uint16_t handle, errcode_t status) {
  unused(uuid);
  sample_print(
      "%s add service cbk callback server_id:%x, handle:%x, status:%x\r\n",
      SLE_KEYBOARD_SERVER_LOG, server_id, handle, status);
}
static void ssaps_add_property_cbk(uint8_t server_id, sle_uuid_t *uuid,
                                   ...
                                   errcode_t status) {
  unused(uuid);
  sample_print("%s add property cbk callback server_id:%x, "
               "service_handle:%x,handle:%x, status:%x\r\n",
               SLE_KEYBOARD_SERVER_LOG, server_id, service_handle, handle,
               status);
}
static void ssaps_add_descriptor_cbk(uint8_t server_id, sle_uuid_t *uuid,
  ...
  sample_print(
      "%s add descriptor cbk callback server_id:%x, service_handle:%x, property_handle:%x, \
                 status:%x\r\n",
      SLE_KEYBOARD_SERVER_LOG, server_id, service_handle, property_handle,
      status);
}
static void ssaps_delete_all_service_cbk(uint8_t server_id, errcode_t status) {
  sample_print("%s delete all service callback server_id:%x, status:%x\r\n",
               SLE_KEYBOARD_SERVER_LOG, server_id, status);
}
static errcode_t
sle_ssaps_register_cbks(ssaps_read_request_callback ssaps_read_callback,
...
static errcode_t sle_uuid_server_property_add(void) {
  ...
  if (memcpy_s(descriptor.value, sizeof(ntf_value), ntf_value,
               sizeof(ntf_value)) != EOK) {
    osal_vfree(property.value);
...
static errcode_t sle_keyboard_server_add(void) {
  ...
  sample_print("%s sle keyboard add service, server_id:%x, service_handle:%x, "
               "property_handle:%x\r\n",
               SLE_KEYBOARD_SERVER_LOG, g_server_id, g_service_handle,
               g_property_handle);
...
errcode_t sle_keyboard_server_send_report_by_uuid(const uint8_t *data,
                                                  uint8_t len) {
  errcode_t ret;
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print("%s "
                 "sle_keyboard_server_send_report_by_uuid,ssaps_notify_"
                 "indicate_by_uuid fail :%x\r\n",
                 SLE_KEYBOARD_SERVER_LOG, ret);
    osal_vfree(param.value);
    return ret;
  }
...
errcode_t sle_keyboard_server_send_report_by_handle(const uint8_t *data,
                                                    uint8_t len) {
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print(
        "%s sle_keyboard_server_send_report_by_handle,ssaps_notify_indicate "
        "fail :%x\r\n",
        SLE_KEYBOARD_SERVER_LOG, ret);
    osal_vfree(param.value);
    return ret;
...
static void sle_connect_state_changed_cbk(uint16_t conn_id,
  ...
  sample_print(
      "%s connect state changed callback addr:%02x:**:**:**:%02x:%02x\r\n",
      SLE_KEYBOARD_SERVER_LOG, addr->addr[BT_INDEX_0], addr->addr[BT_INDEX_4]);
  if (conn_state == SLE_ACB_STATE_CONNECTED) {
    ...
    if (g_sle_keyboard_server_msg_queue != NULL) {
      g_sle_keyboard_server_msg_queue(sle_connect_state,
                                      sizeof(sle_connect_state));
    }
  }
}
...
static errcode_t sle_conn_register_cbks(void) {
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print("%s sle_conn_register_cbks,sle_connection_register_callbacks "
                 "fail :%x\r\n",
                 SLE_KEYBOARD_SERVER_LOG, ret);
    return ret;
  }
...
uint16_t sle_keyboard_client_is_connected(void) { return g_sle_pair_handle; }

/* 初始化uuid server */
errcode_t
sle_keyboard_server_init(ssaps_read_request_callback ssaps_read_callback,
                         ssaps_write_request_callback ssaps_write_callback) {
  errcode_t ret;
  ret = enable_sle();
  if (ret != ERRCODE_SLE_SUCCESS) {
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print(
        "%s sle_keyboard_server_init,sle_conn_register_cbks fail :%x\r\n",
        SLE_KEYBOARD_SERVER_LOG, ret);
    return ret;
  }
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print(
        "%s sle_keyboard_server_init,sle_ssaps_register_cbks fail :%x\r\n",
        SLE_KEYBOARD_SERVER_LOG, ret);
    return ret;
  }
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print(
        "%s sle_keyboard_server_init,sle_keyboard_server_add fail :%x\r\n",
        SLE_KEYBOARD_SERVER_LOG, ret);
    return ret;
  }
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print(
        "%s sle_keyboard_server_init,sle_keyboard_server_adv_init fail :%x\r\n",
        SLE_KEYBOARD_SERVER_LOG, ret);
    return ret;
  }
...
void sle_keyboard_server_register_msg(
    sle_keyboard_server_msg_queue sle_keyboard_server_msg) {
