...
static int sle_set_default_announce_param(void) {
  ...
  if (memcpy_s(param.own_addr.addr, SLE_ADDR_LEN, local_addr, SLE_ADDR_LEN) !=
      EOK) {
...
static int sle_set_default_announce_data(void) {
  ...
  for (uint8_t data_index = 0; data_index < data.announce_data_len;
       data_index++) {
  ...
  seek_data_len =
      sle_set_scan_response_data(seek_rsp_data, SLE_ADV_DATA_LEN_MAX);
  ...
  for (uint8_t data_index = 0; data_index < data.seek_rsp_data_len;
       data_index++) {
...
errcode_t sle_keyboard_announce_register_cbks(void) {
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print("%s sle_keyboard_announce_register_cbks,register_callbacks "
                 "fail :%x\r\n",
...
errcode_t sle_keyboard_server_adv_init(void) {
  ...
  if (ret != ERRCODE_SLE_SUCCESS) {
    sample_print(
        "%s sle_keyboard_server_adv_init,sle_start_announce fail :%x\r\n",
        SLE_KEYBOARD_SERVER_LOG, ret);
    return ret;
  }
  return ERRCODE_SLE_SUCCESS;
