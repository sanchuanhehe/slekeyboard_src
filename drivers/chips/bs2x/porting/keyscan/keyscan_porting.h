...
#define KEYSCAN_MAX_KEY_NUM                                                    \
  (KEYSCAN_ROW_NUM + KEYSCAN_MAX_COL) // TODO:CUSTOME KEYS
#define KEYSCAN_USED_NUM                                                       \
  (KEYSCAN_ROW_NUM + KEYSCAN_COL_NUM) // TODO:CUSTOME KEYS
 ...
 */
typedef enum {
  ROW_SCAN_DIR, /*!< 0: Row is configured as output, Column is configured as
                   input */
  COL_SCAN_DIR  /*!< 1: Column is configured as output, Row is configured as
                   input */
} keyscan_dir_t;
 ...
 */
typedef enum {
  HAL_KEYSCAN_MODE_0, /*!< Input=0 Output=1 Pull down Output while scanning,
                         scan high level */
  HAL_KEYSCAN_MODE_1, /*!< Input=1 Output=0 Pull up Output while scanning, scan
                         low level */
  HAL_KEYSCAN_MODE_2, /*!< Input=0 Output=1 Pull up Input while scanning, scan
                         low level */
  HAL_KEYSCAN_MODE_3  /*!< Input=1 Output=0 Pull down Input while scanning, scan
                         high level */
} keyscan_mode_t;
...
/**
 * @brief  Key scan change row/col waiting time, contain CBB external
 * observation time and internal path time.
 */
typedef enum {
...
typedef enum {
  ...
  CUSTOME_KEYS_TYPE /*!< KEYSCAN costom keys keyboard */
} keyscan_keys_type_t;
...
#define KEYSCAN_DEFAULT_CONFIG                                                 \
  {                                                                            \
    .direction = ROW_SCAN_DIR, .scan_mode = HAL_KEYSCAN_MODE_0,                \
    .pulse_time = EVERY_ROW_PULSE_10_US, .wait_time = WAITING_30_AND_5_US,     \
    .idle_time = IDLE_3_MS_AFTER_SCAN, .defence_time = SAMPLE_NONE_DEFENCE,    \
    .select_num = SELECT_SIX_KEYS_PER_SCAN, .ghost_check = false,              \
    .io_de = true,                                                             \
  }
 ...
 */
typedef struct {
  keyscan_dir_t direction; /*!< @if Eng Indicate whether row or column be
                              output, the other be input.
                                @else 表明当行或列其中一个为输出时，另一个为输入
                              @endif */
  keyscan_mode_t
      scan_mode; /*!< @if Eng Indicate level of input and output in initial
                    state and scanning.
                      @else   声明初始状态和扫描过程的输入、输出电平 @endif */
  keyscan_pulse_time_t
      pulse_time; /*!< @if Eng Duration of the pulse as each row is scanning,
                              depends on the load.
                       @else   每行扫描时脉冲的持续时间，取决于负载 @endif */
  keyscan_wait_time_t
      wait_time; /*!< @if Eng Key scan change row/col waiting time.
                      @else   键扫描更改行/列等待时间 @endif */
  keyscan_idle_time_t
      idle_time; /*!< @if Eng No scanning during this time, keys restoring the
                    initial state.
                      @else   在此期间不进行扫描，按键恢复初始状态 @endif */
  key_defence_num_t
      defence_time; /*!< @if Eng Sample defence nums, get the same coordinate
                       means its vaild Config defence_time and idle_time to
                       confirm that scanning time is bigger than jitter time.
                         @else
                       采样防御数，获取相同坐标表示其有效配置防御时间和空闲时间，
                                以确认扫描时间大于抖动时间 @endif */
  key_select_num_t select_num; /*!< @if Eng How many coordinates of key can be
                                  identified during one time.
                                    @else   一次可以识别的键坐标数 @endif */
  bool ghost_check; /*!< @if Eng If three keys form a rectangle, only report two
                       keys' coordinates.
                         @else   如果三个键组成一个矩形，只报告两个键的坐标
                       @endif */
 ...
 */
void keyscan_porting_config_pins(pin_t *gpio_map, uint8_t gpio_num);
