 ...
 *
 * @note  前  @ref KEYSCAN_MAX_COL 个为列，后  @ref KEYSCAN_MAX_ROW 个为行
 */
pin_t g_gpio_map[KEYSCAN_GPIO_NUM] = {2,  3,  4,  5,  6,  21, 9,  29,
                                      31, 24, 14, 23, 27, 28, 10, 11,
                                      30, 13, 15, 16, 25, 26, 12, 22};
...
uintptr_t keyscan_porting_base_addr_get(void) { return g_keyscan_base_addr; }
...
#if defined(CONFIG_KEYSCAN_SUPPORT_SLEEP)
  osal_irq_request(KEY_SCAN_LOW_POWER_IRQN, irq_keyscan_slp_handler, NULL, NULL,
                   NULL);
  osal_irq_set_priority(KEY_SCAN_LOW_POWER_IRQN,
                        irq_prio(KEY_SCAN_LOW_POWER_IRQN));
  osal_irq_enable(KEY_SCAN_LOW_POWER_IRQN);
  uapi_timer_start(g_keyscan_timer, CONFIG_KEYSCAN_IDLE_WAIT_US,
                   keyscan_timer_irq, 1);
#endif
}
...
void keyscan_porting_pin_set(void) {
  ...
  if (g_keyboard_type_sel == FULL_KEYS_TYPE) {
    ...
    for (int i = 0; i <= NUM_OF_CFG_PIN_REGS; i++) {
      reg16_setbits(KEYSCAN_PIN_SEL_BASE_ADDR + (i * INTERVAL_OF_REGS),
                    EVEN_SEL_BIT, BIT_SEL_LEN, g_gpio_map[gpio_index]);
      gpio_index++;
      reg16_setbits(KEYSCAN_PIN_SEL_BASE_ADDR + (i * INTERVAL_OF_REGS),
                    ODD_SEL_BIT, BIT_SEL_LEN, g_gpio_map[gpio_index]);
      gpio_index++;
    }
    ...
    reg16_setbits(KEYSCAN_SIX_TYPE_COL_REG, EVEN_SEL_BIT, BIT_SEL_LEN,
                  g_gpio_six_map[gpio_index++]);
    reg16_setbits(KEYSCAN_SIX_TYPE_COL_REG, ODD_SEL_BIT, BIT_SEL_LEN,
                  g_gpio_six_map[gpio_index++]);
    reg16_setbits(KEYSCAN_SIX_TYPE_ROW0_REG, EVEN_SEL_BIT, BIT_SEL_LEN,
                  g_gpio_six_map[gpio_index++]);
    reg16_setbits(KEYSCAN_SIX_TYPE_ROW0_REG, ODD_SEL_BIT, BIT_SEL_LEN,
                  g_gpio_six_map[gpio_index++]);
    ...
    for (int i = 0; i < (KEYSCAN_COL_NUM / 2 + 1); i++) {
      reg16_setbits(KEYSCAN_PIN_SEL_COL_BASE_ADDR + (i * INTERVAL_OF_REGS),
                    EVEN_SEL_BIT, BIT_SEL_LEN, g_gpio_map[gpio_index]);
      gpio_index++;
    ...
    for (int i = 0; i < (KEYSCAN_ROW_NUM / 2 + 1); i++) {
      reg16_setbits(KEYSCAN_PIN_SEL_ROW_BASE_ADDR + (i * INTERVAL_OF_REGS),
                    EVEN_SEL_BIT, BIT_SEL_LEN, g_gpio_map[gpio_index]);
      gpio_index++;
